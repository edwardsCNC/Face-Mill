Face-Mill is a CNC programming utility that creates a climb milling CNC program which can be used to machine a rectangular flat surface. The output CNC program can be interpreted by a 3-axis CNC machine running [GRBL](https://github.com/grbl/grbl), and the toolpath can be visually previewed at [Q'n'dirty toolpath simulator](https://nraynaud.github.io/webgcode/).
...
[Click here for a link to a video sample](https://www.youtube.com/watch?v=xK7AqyGdtgA) of a CNC router running a program generated by Face-Mill.
...
In order to run Face-Mill and generate a CNC program, you must first be able to navigate your computer's file directory and execute commands from a shell, and you must have Ruby 2.0.0 installed. Other Ruby versions may work as well, but are untested.

To run Face-Mill, first download the .zip file and extract the contents to a directory of your choice. Then, access that directory with your shell, navigate into the ./bin directory, and execute:
